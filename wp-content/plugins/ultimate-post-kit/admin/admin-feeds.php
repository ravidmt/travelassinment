...
class Ultimate_Post_Kit_Admin_Feeds {
    ...
    function enqueue_product_feeds_styles($hook) {
        ...
        wp_enqueue_style('upk-product-feed', BDTUPK_URL . 'admin/assets/css/upk-product-feed' . $direction_suffix . '.css', [], BDTUPK_VER);
    }
    ...
    public function ultimate_post_kit_register_rss_feeds() {
        wp_add_dashboard_widget('bdt-upk-dashboard-overview', esc_html__('Ultimate Post Kit News &amp; Updates', 'ultimate-post-kit'), [$this, 'ultimate_post_kit_rss_feeds_content_data'], null, null, 'column4', 'core');
    }
    ...
    public function ultimate_post_kit_get_feeds_posts_data() {
        ...
        <div class="bdt-upk-overview__feed">
            <ul class="bdt-upk-overview__posts">
                ...
                if ($maxitems == 0) {
                    echo '<li class="bdt-upk-overview__post">' . __('No item', 'ultimate-post-kit') . '.</li>';
                } else {

                    foreach ($rss_items as $item) :
                        $feed_url     = $item->get_permalink();
                        $feed_title   = $item->get_title();
                        $feed_date    = human_time_diff($item->get_date('U'), current_time('timestamp')) . ' ' . __('ago', 'ultimate-post-kit');
                        $content      = $item->get_content();
                        $feed_content = wp_html_excerpt($content, 120) . ' [...]';
                ?>
                        <li class="bdt-upk-overview__post">
                            <?php printf('<a class="bdt-upk-overview__post-link" href="%1$s" title="%2$s" target="_blank">%3$s</a>', $feed_url, $feed_date, $feed_title);
            ...
            <div class="bdt-upk-overview__footer bdt-upk-divider_top">
                <ul>
                    ...
                    $footer_link = [
                        ...
                        [
                            'url'   => 'https://bdthemes.com/ultimate-post-kit/#purchase-section',
                            'title' => esc_html__('Get Pro', 'ultimate-post-kit'),
                        ],
                        [
                            'url'   => 'https://postkit.frill.co/announcements/',
